            ...
            <div class="container-fluid p-4">
                ...
                <?php if (!empty($available_years)): ?>
                    <div class="sy-filter-bar mb-3">
                        <span class="sy-filter-label"><i class="bi bi-calendar-range"></i> School Year</span>
                        <div class="dropdown">
                            <button class="btn btn-sm btn-primary dropdown-toggle sy-dropdown-btn" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                                <i class="bi bi-calendar-check"></i> <?= htmlspecialchars($school_year) ?>
                            </button>
                            <ul class="dropdown-menu sy-dropdown-menu shadow-sm">
                                <?php foreach ($available_years as $year): ?>
                                    <li><a class="dropdown-item <?= $school_year === $year ? 'active' : '' ?>" href="index.php?page=grading&school_year=<?= urlencode($year) ?>&semester=<?= urlencode($semester) ?>"><i class="bi bi-<?= $school_year === $year ? 'check2' : 'calendar3' ?> me-1"></i> <?= htmlspecialchars($year) ?></a></li>
                                <?php endforeach; ?>
                            </ul>
                        </div>
                        <span class="sy-year-count"><?= count($available_years) ?> school year(s) available</span>
                    </div>
                <?php endif; ?>

                <div class="card year-levels-card">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="bi bi-layers-fill"></i> Available Year Levels <?php if (!empty($school_year)): ?><span class="badge bg-secondary ms-2"><?= htmlspecialchars($school_year) ?></span><?php endif; ?></h5>
                    </div>
                    <div class="card-body">
                        ...
                        <?php else: ?>
                            <div class="year-levels-grid">
                                <?php foreach ($year_levels as $level): ?>
                                    <a href="index.php?page=grading_subjects&year_level=<?= urlencode($level['year_level']) ?>&school_year=<?= urlencode($school_year) ?>&semester=<?= urlencode($semester) ?>" class="year-level-card">
                                        <div class="year-level-icon"><i class="bi bi-bookmarks-fill"></i></div>
                                        ...
                                        <div class="year-level-arrow"><i class="bi bi-arrow-right"></i></div>
                                    </a>
                                <?php endforeach; ?>
                            </div>
                        <?php endif; ?>
